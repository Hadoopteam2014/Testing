   ...
   if oauth_verifier:
    ...
    for i in range(edulen):
     try:
      ...
      p2=usereducation(Userids = x['id'],eduid=eduid,fieldofstudy=fieldofstudy,school=schl,degree=deg,startDate=startDate,endDate=endDate)
    ...
    for j in range(x1['_count']):
     try:
      ...
      p7 = company(updateKey=updateKey,updatecontent=updatecontent,companyid=compid,timestamp=timestamp,numLikes=numLikes,updatecomments=updatecomments)
